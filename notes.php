    ...
    if (mysqli_num_rows($user_verification_result) == 1) {
        ...
        if ($user_status != 1) {
            ...
            if (isset($_POST['search'])) {
                ...
                $search = $searchID;
                $modle_paper = "SELECT * FROM `modle_papers_&_tutes` WHERE (`Title` LIKE '%{$searchID}%' OR `File_name` LIKE '%{$searchID}%' OR `Date_Time` LIKE '%{$searchID}%') AND `Class` = {$_SESSION['Class']} AND `Category` = 6 AND `Status` = 1";
            } else {
                $modle_paper = "SELECT * FROM `modle_papers_&_tutes` WHERE `Class` IN(1 , {$_SESSION['Class']}) AND `Category` = 6 AND `Status` = 1";
            }
...
<head>
    ...
    <title>Mr.ChemistrY - Notes</title>
