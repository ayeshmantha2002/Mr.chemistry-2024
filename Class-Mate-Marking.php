    ...
    if (mysqli_num_rows($user_verification_result) == 1) {
        ...
        if ($user_status != 1) {
            ...
            if (isset($_POST['search'])) {
                ...
                $search = $searchID;
                $modle_paper = "SELECT * FROM `modle_papers_&_tutes` WHERE (`Title` LIKE '%{$searchID}%' OR `File_name` LIKE '%{$searchID}%' OR `Date_Time` LIKE '%{$searchID}%') AND `Class` IN(1 , {$_SESSION['Class']}) AND `Category` = 5 AND `Status` = 1";
            } else {
                $modle_paper = "SELECT * FROM `modle_papers_&_tutes` WHERE `Class` IN(1 , {$_SESSION['Class']}) AND `Category` = 5 AND `Status` = 1";
            }
...
<head>
    ...
    <title>Mr.ChemistrY - ClassMate Marking</title>
...
<body>
    ...
    <div id="upNav">
        <div class="upNav">
            ...
            <ul>
                <li><a href="Class-Mate" onclick='loadinEffect()'> CM-Papers </a></li>
            </ul>
        </div>
    </div>
